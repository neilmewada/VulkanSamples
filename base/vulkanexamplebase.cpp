...
void VulkanExampleBase::setupFrameBuffer()
{
	...
	for (uint32_t i = 0; i < offscreenImages.size(); i++)
	{
		...
		VkMemoryRequirements memReqs{};
		vkGetImageMemoryRequirements(device, offscreenImages[i], &memReqs);
		...
		VK_CHECK_RESULT(vkAllocateMemory(device, &memAlloc, nullptr, &offscreenImageMemories[i]));
		VK_CHECK_RESULT(vkBindImageMemory(device, offscreenImages[i], offscreenImageMemories[i], 0));
...
void VulkanExampleBase::windowResize()
{
	...
	for (auto& memory : offscreenImageMemories)
	{
